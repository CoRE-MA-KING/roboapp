...
CollisionAvoidance::CollisionAvoidance(double robot_width, double robot_length,
  ...
  corners_ = {cv::Point2d(-robot_width_ / 2.0, -robot_length_ / 2.0),
    ...
    for (const auto& pt : lidar_points) {
      ...
      if (dist < influence_range_ + eps) {
        ...
        double repulsive_force =
            repulsive_gain_ *
            std::exp(-pow(safe_dist, 2) / (2 * pow(sigma, 2)));
